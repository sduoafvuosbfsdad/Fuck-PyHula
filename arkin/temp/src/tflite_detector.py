...
class tflite_detector:
    def draw_center_cross(self, frame, x, y, color=(0, 0, 255), size=10, thickness=2):
    ...
    def __init__(self,model="model.tflite",label="label.txt",vid_width=1280,vid_height=720):
        self.confidence_level = 0.38 #EDIT CONFIDENCE THRESHOLD
...
#        scores = self.interpreter.get_tensor(self.output_details[2]['index']).squeeze()
    ...
    def get_highest_confidence_center(self, frame=None):
        highest_score_obj, _ = self.detect(frame)
        if highest_score_obj is not None:
            return highest_score_obj['x'], highest_score_obj['y']
        return None, None
