...
class PostProcessors:
    def __init__(self):
        ...
        self.ContourRed = self.ContourRed()
    ...
    class ContourRed:
        def __init__(self):
            # Initialization can include any parameters if needed
            pass

        def __call__(self, frame):
            # Define HSV range for red color
            Lred1 = np.array([0, 150, 70])  # Lower range for red (hue ranges to cover red wrap around 0)
            Ured1 = np.array([10, 255, 255])
            Lred2 = np.array([170, 150, 70])  # Second range for red (higher wrap around)
            ...
            mask_red = mask_red1 | mask_red2  # Combine both masks for full red detection
